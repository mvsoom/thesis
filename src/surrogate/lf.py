...
from surrogate import holmberg, period
from utils import __memory__, bijectors, constants, lfmodel
...
RK_BOUNDS = [constants._ZERO, 1.0]  # Is a percentage
...
def sample_Rk(Re, rng):
    ...
    """
    Rk = _UNCONTAINED
    while not _contains(RK_BOUNDS, Rk):
        Rk = (22.4 + 11.8 * Re) / 100 + RK_STDDEV * rng.normal()
    return Rk
...
RA_BOUNDS = [constants._ZERO, 1.0]  # Is a percentage
...
def sample_Ra(Re, rng):
    ...
    """
    Ra = _UNCONTAINED
    while not _contains(RA_BOUNDS, Ra):
        Ra = (-1 + 4.8 * Re) / 100 + RA_STDDEV * rng.normal()
    return Ra
    ...
    * Note that `Rg` can be larger than 1, unlike `Ra `and `Rg` (Fant 1994, Fig. 3)
    ...
    if not _contains(RE_BOUNDS, Re):
        ...
        while not _contains(RG_BOUNDS, Rg):
            ...
            Rk = sample_Rk(Re, rng)
            Rg = (
                Rk * (0.5 + 1.2 * Rk) / (0.44 * Re - 4 * Ra * (0.5 + 1.2 * Rk))
            )  # Uncertainty from Ra and Rk transfers to Rg
    return Ra, Rk, Rg
...
@__memory__.cache
def sample_lf_params(fs=constants.FS_KHZ, numsamples=int(1e5), seed=44178):
    """Sample the `R`, `T` and `generic` parameters of the LF model

    This will return way less samples than `numsamples`, because we emply rejection
    ...
    rng_seed = int(
        jax.random.randint(key1, (1,), minval=0, maxval=int(1e4)).item()
    )
    ...
    p = _collect_list_of_dicts(
        [sample_R_params(T, Td, rng) for T, Td in zip(T0, Td)]
    )
    ...
    T0_max = jnp.max(T0)
    t = jnp.linspace(0.0, T0_max, int(T0_max * fs) + 1)
    ...
    def calc_dgf_power(p):
        ...
        u = jnp.where(
            u, u, jnp.nan
        )  # Ignore points outside of the pitch period
...
def sample_and_log_prob_dgf(
    ...
    tmax = bufsize / fs
    tbuffer = jnp.linspace(0.0, tmax, bufsize)

    def sample_normalized_dgf_and_log_prob(p, key, sigma=sigma, tol=1e-6):
        ...
        def normalized_dgf(p):
            u0 = lfmodel.dgf(
                tbuffer, p, tol=tol, initial_bracket=initial_bracket
            )
        ...
        def normalized_dgf_jacobian(p):
            ...
            jacobian = jnp.vstack(
                [jacobian_dict[k] for k in constants.LF_T_PARAMS]
            ).T  # (N, P)
            return jacobian
    ...
    vmapped = jax.vmap(sample_normalized_dgf_and_log_prob, in_axes=[0, 0])
...
def closed_phase_leading(t, u, p, treshold, offset=0.0):
    """Take a DGF waveform and put its closed phase before the GOI"""
    i = _nonzero_dgf_indices(t - offset, p["T0"])
    t_nonzero = t[i]
    u_nonzero = u[i]
    u_nonzero_swapped = _swap_dgf(
        t_nonzero - offset, u_nonzero, p["Te"], treshold
    )
