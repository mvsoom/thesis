...
from utils import __datadir__, bijectors, constants


def read(s):
    df = pd.read_csv(__datadir__(f"holmberg/{s}-normal-loudness.csv"))
    df["sex"] = s
...
def declination_time_prior(cacheid=45870):
    ...
    prior = tfd.TransformedDistribution(standardnormal, bijector)
