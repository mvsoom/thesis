...
import utils
from surrogate import lf, period
from utils import __cache__, __memory__, bijectors, constants, isokernels
...
def get_lf_samples(
    ...
    samples = [sample(key) for key in tqdm(keys)]
    return samples
...
def fit_lf_sample(
    ...
    def ptform(u, prior=scipy.stats.lognorm(np.log(10))):
        if not use_oq:
...
def yield_fitted_lf_samples(
    seed=67011, kernel_ms=KERNEL_MS, kernel_names=KERNEL_NAMES, verbose=False
):
            ...
            for kernel_name in kernel_names:
                for use_oq in (False, True):
                    for impose_null_integral in (False, True):
                        ...
                        yield dict(
                            i=i, sample=sample, config=config, results=results
                        )
...
def process_fitted_lf_samples():
    def process():
        for fit in yield_fitted_lf_samples():
            ...
            mean, _ = utils.get_posterior_moments(results)
            mean_dict = {k: mean[i] for i, k in enumerate(FIT_LF_SAMPLE_PARAMS)}
...
def posterior_of_fitted_lf_values(selection=[], numsamples=100):
    def process():
        for fit in yield_fitted_lf_samples():
            ...
            yield utils.resample_equal(results, numsamples)
...
def fit_source_bijector(config, return_fit_results=False):
    ...
    return bijectors.fit_nonlinear_coloring_bijector(
        pruned, bounds, cacheid, return_fit_results=return_fit_results
    )
...
def trajectify_bijector(bstatic, num_pitch_periods, constant=False):
    ...
    kernel_name, _, results = period.fit_period_trajectory_kernel()
...
def source_trajectory_bijector(
    ...
    trajectory_bijector = trajectify_bijector(
        marginal_bijector, num_pitch_periods, constant=constant
    )
    ...
    trajectory_bijector = (
        bijectors.condition_nonlinear_coloring_trajectory_bijector(
            trajectory_bijector,
            observation,
            noise_cov,
            noise_mean,
...
def source_trajectory_prior(
    num_pitch_periods, config, T_estimate=None, noiseless_estimates=False
):
    b = source_trajectory_bijector(
        num_pitch_periods, config, T_estimate, noiseless_estimates
    )
    ...
    ndim = SOURCE_NDIM(config)
    standardnormals = tfd.MultivariateNormalDiag(
        scale_diag=jnp.ones(num_pitch_periods * ndim)
    )

    prior = tfd.TransformedDistribution(
        distribution=standardnormals, bijector=b, name=name
    )
    return prior  # prior.sample(ns) shaped (ns, num_pitch_periods, ndim)


def source_marginal_prior(config, T_estimate=None, noiseless_estimates=False):
    ndim = SOURCE_NDIM(config)
    squeeze_bijector = tfb.Reshape(
        event_shape_out=(ndim,), event_shape_in=(1, ndim)
    )
    prior = tfd.TransformedDistribution(
        distribution=source_trajectory_prior(
            1, config, T_estimate, noiseless_estimates
        ),
...
def get_source_envelope_kernel():
    envelope_kernel_name, _, results = period.fit_period_trajectory_kernel()

    envelope_lengthscale, _ = period.maximum_likelihood_envelope_params(results)
