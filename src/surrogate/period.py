...
def split_markings_into_voiced_groups(
    m, fs, max_period_length_msec, min_num_periods
):
...
def yield_training_pairs(recordings, markings, return_praat_gci_error=False):
    ...
    for key in recordings.keys():
        ...
        for true_group in voiced_groups:
            ...
            true_group, praat_group = align_and_intersect(
                true_group, praat_pulses
            )
            ...
            if np.any(praat_periods > max_period_length_msec) or np.any(
                praat_periods < min_period_length_msec
            ):
...
def fit_period_trajectory_kernel():
    ...
    def fit(kernel_name, cacheid):
        bijector, results = (
            bijectors.fit_nonlinear_coloring_trajectory_bijector(
...
def fit_period_trajectory_bijector(num_pitch_periods=None):
...
def fit_praat_estimation_mean_and_cov():
    ...
    mean, cov = bijectors.estimate_observation_noise_cov(
        b, true_samples, praat_samples, return_mean=True
    )
    return mean, cov
...
def period_trajectory_prior(num_pitch_periods, praat_estimate=None):
    """
    ...
    standardnormals = tfd.MultivariateNormalDiag(
        scale_diag=jnp.ones(num_pitch_periods)
    )

    # Squeeze out the last dimension in `(m,n)` since `n == 1`
    squeeze_bijector = tfb.Chain(
        [tfb.Reshape(event_shape_out=(-1,), event_shape_in=(-1, 1)), bijector]
    )

    prior = tfd.TransformedDistribution(
        distribution=standardnormals, bijector=squeeze_bijector, name=name
    )
