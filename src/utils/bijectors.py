...
from utils import __memory__, isokernels
...
def softclipexp_bijector(bounds, sigma):
    ...
    """
    return tfb.Chain(
        [tfb.Exp(), tfb.SoftClip(bounds[:, 0], bounds[:, 1], sigma)]
    )
...
def vec_to_matrix_bijector(m, n, constant=False):
    ...
    if not constant:
        ...
        reshape = tfb.Reshape(
            event_shape_out=(n, m),
            event_shape_in=(n * m,),  # A matrix  # A vector
        )
...
def nonlinear_coloring_trajectory_bijector(
    ...
    if constant:
        ...
        return tfb.Chain(
            [softclipexp, vec_to_matrix_bijector(m, n, constant=True), color]
        )
    ...
    envelope_tril = jnp.linalg.cholesky(
        stabilize(envelope_K, envelope_noise_sigma)
    )
    ...
    return tfb.Chain(
        [softclipexp, vec_to_matrix_bijector(m, n, constant=False), color]
    )
...
def list_to_batches(samples):
    ...
    batches = [
        jnp.stack(list(g)) for _, g in itertools.groupby(samples, key=len)
    ]
    return batches
...
def batches_to_list(batches):
    ...
    return [
        ...
        for sample in jnp.split(batch, len(batch))
    ]
...
def estimate_observation_noise_cov(
    ...
    **Note: the covariance is estimate in the "colored domain", i.e., where
    ...
    softclipexp, vec_to_matrix, color = (
        nonlinear_coloring_trajectory_bijector.bijectors
    )

    error = softclipexp.inverse(observed_stacked) - softclipexp.inverse(
        true_stacked
    )
...
def drop_dimensions(
    nonlinear_coloring_trajectory_bijector, dropdims=[], constant=False
):
    """
    ...
    #####################################
    new_vec_to_matrix = vec_to_matrix_bijector(
        m, jnp.int32(new_n), constant=constant
    )
