"""Legacy isotropic kernels for code preceding tinygp"""
...
from tensorflow_probability.substrates.jax.math import (
    psd_kernels as jax_psd_kernels,
)
...
class IsoKernel:
    ...
    def spectral_density(self, s, inline=True):
        return (
            (self.amplitude**2)
            * self.length_scale
            * self._unscaled_spectral_density(self.length_scale * s)
        )
...
class CustomIsoKernel(IsoKernel, jax_psd_kernels.ExponentiatedQuadratic):
    ...
    def _apply_with_distance(
        ...
        from tensorflow_probability.python.internal.backend.jax.compat import (
            v2 as tf,
        )
        from tensorflow_probability.substrates.jax.math.psd_kernels.internal import (
            util,
        )
...
def spectral_density_matern12(s):
    return 2 / (1 + s**2)
...
def spectral_density_matern32(s):
...
def spectral_density_matern52(s):
...
def spectral_density_sinc(s):
    return jnp.where(s <= jnp.pi, 1.0, 0.0)
...
def spectral_density_compactpoly0(s):
    return jnp.sin(jnp.pi * s) ** 2 / (jnp.pi * s) ** 2
...
def spectral_density_compactpoly1(s):
    return (
        6 * jnp.pi * s * (2 + jnp.cos(2 * jnp.pi * s))
        - 9 * jnp.sin(2 * jnp.pi * s)
    ) / (2 * jnp.pi**5 * s**5)
...
def spectral_density_compactpoly2(s):
    return (
        ...
        * (
            -12
            + 8 * jnp.pi**2 * s**2
            - 2 * (-6 + jnp.pi**2 * s**2) * jnp.cos(2 * jnp.pi * s)
            + 9 * jnp.pi * s * jnp.sin(2 * jnp.pi * s)
        )
        / (4 * jnp.pi**8 * s**8)
    )
...
def spectral_density_compactpoly3(s):
    return (
        945
        * (
            64 * jnp.pi * s * (-6 + jnp.pi**2 * s**2)
            + 2
            * jnp.pi
            * s
            * (-123 + 4 * jnp.pi**2 * s**2)
            * jnp.cos(2 * jnp.pi * s)
            + 9 * (35 - 8 * jnp.pi**2 * s**2) * jnp.sin(2 * jnp.pi * s)
        )
    ) / (4 * jnp.pi**11 * s**11)
