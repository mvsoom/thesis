...
FRAME_LENGTH_SEC = 10 / 1000.0  # 10 msec
...
def yield_file_triples(vtr_root, timit_root):
    ...
    for fb_file in vtr_root.rglob("*.FB"):
        ...
        if not wav_file.is_file():
            warnings.warn(
                f"No WAV file at {wav_file} found for {fb_file}; skipping"
            )
            continue
        phn_file = corresponding_phn(fb_file, vtr_root, timit_root)
        if not phn_file.is_file():
            warnings.warn(
                f"No PHN file at {phn_file} found for {fb_file}; skipping"
            )
            continue
