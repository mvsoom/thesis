...
They fit into the same piecewise framework we have been using for the LF model, but with each part modeled as a polynomial of maximum degree $d$.
We list several in @table:polys.

These GFMs mainly appeal to simplicity and spectral strength.
They models satisfy the closure constraint analytically and remain computationally cheap.
Importantly, the sharp transitions generated by low-order polynomials produce a bright spectrum with slow decay, making them efficient exciters for the vocal tract @Childers1994.
