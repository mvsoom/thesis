...
% This is added so that we ensure that the Seg variable below can always be
% extracted (indices never go out of wave)
T0=round(Fs/min(f02));
gcis(gcis<round(0.9*T0)+1) = [];
gcis(gcis>length(wave)-round(0.9*T0)) = [];
for k=1:length(gcis)
