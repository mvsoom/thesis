...
logp = t_log_evidence_is_on_test(qsvi, test_data, subkey)
...
logp_null = t_log_evidence_is_on_test(qsvi_null, test_data, subkey)

print(
    "Average log likelihood per effective data point on test set under NULL:",
    np.mean(logp_null / neff),
)
...
mean_loglike_test = jnp.mean(logp / neff)
mean_loglike_test_null = jnp.mean(logp_null / neff)