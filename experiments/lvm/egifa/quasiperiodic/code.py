...
M = 4  # Number of PRISM basis functions
...
from prism.harmonic import SGMQuasiPeriodic, SHMPeriodic
...
def collapsed_svi(key=vk(), M=M, num_harmonics=num_harmonics):
    am = SGMRBF(variance=1.0, lengthscale=1.0)
    periodic = SHMPeriodic(
        variance=1.0, lengthscale=1.0, period=1.0, num_harmonics=num_harmonics
    )

    kernel = SGMQuasiPeriodic(am, periodic)

    prior = gpx.gps.Prior(kernel, Zero())
    likelihood = Gaussian(num_datapoints=WIDTH)
    posterior = prior * likelihood

    return SGMCollapsedVariationalGaussian(posterior=posterior, M=M)
...
print("Learned kernel lengthscale:", qsvi.posterior.prior.kernel.lengthscale)
...
eps = jax.random.normal(subkey, shape=(2 * M + 1, 5))
y = Psi_test @ eps
...
svi_variance = float(qsvi.posterior.prior.kernel.variance)
...
def process_K(K, key=vk()):
    ...
    plot_cluster_samples_in_data_space(key, qgp, tau_test, nsamples=3).show()
