...
##############################################################
batch_size = 32
num_iters = 5_000  # seems enough
lr = 1e-3
jitter = 1e-4


def collapsed_svi(key=vk(), d=d, J=J, M=M):
    kernel = SHMPeriodicFFT(
        PACK(d=d, J=J, period=1.0, variance=1.0, weights=None),
        num_harmonics=M,
    )
...
print("Learned kernel variance:", qsvi.posterior.prior.kernel.kernel.variance)
print("Learned kernel weights:", qsvi.posterior.prior.kernel.kernel.weights)
...
svi_weights = np.array(qsvi.posterior.prior.kernel.kernel.weights)
svi_variance = float(qsvi.posterior.prior.kernel.kernel.variance)
