...
from gpjax.kernels import RBF, ProductKernel
from gpjax.likelihoods import Gaussian
from gpjax.mean_functions import Zero
from gpjax.variational_families import CollapsedVariationalGaussian

from egifa.data import get_test_data, get_train_data
...
def collapsed_svi(key=vk(), d=d, J=1, M=M):
    ...
    kernel = ProductKernel([RBF(), NormalizedPACK(d=d, J=J)])
...
tau_test = jnp.linspace(0, 5, 1000)
...
X_test, Y_test, oq_test = get_test_data(n=N_TEST)
...
def process_K(K, key=vk()):
    ...
    plot_oq_vs_loglik(oq_test, log_prob_gmm / n_eff).show()
