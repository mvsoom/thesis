...
runs_file <- file.path(Sys.getenv("PROJECT_EXPERIMENTS_PATH"), "modal/pack/runs.csv")
...
# styler: off
    by = .(modality, kernel, J, single_sigma_c, normalized)
    # styler: on
]
...
kernel_scores <- df[
    ...
    by = .(kernel, J, single_sigma_c, normalized)
]
