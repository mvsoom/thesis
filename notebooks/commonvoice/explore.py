...
def _load_wav_normalized(path, target_fs=None):
    ...
    if target_fs is not None and fs != target_fs:
        speech = scipy.signal.resample_poly(speech, target_fs, fs).astype(
            np.float32
        )
        fs = target_fs
...
def _full_quickgci(wav_path, fs, cache):
    key = (Path(wav_path), int(fs))
    if key not in cache:
        fs_wav, speech_wav = _load_wav_normalized(key[0], target_fs=int(fs))
        if fs_wav != fs:
            raise ValueError(
                f"Sample rate mismatch for {key[0]}: {fs_wav} != {fs}"
            )
...
def plot_random_selected_window(
    ...
    recording_name = v.get("name", Path(v["wav"]).stem)
