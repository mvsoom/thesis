...
def collapsed_svi(key=vk(), nu=1e6, M=12, jitter=1e-6, lengthscale=1.0, variance=1.0, obs_stddev=1.0, num_inner=3):
    ...
    return t_svi.t_CollapsedVariationalGaussian(
        posterior=posterior, inducing_inputs=Z, nu=nu, num_inner=num_inner, jitter=jitter
    )
...
q = collapsed_svi(key, nu=nu, M=12, num_inner=num_inner, jitter=1e-6)


elbo = svi.collapsed_elbo_masked(q, t, y)
elbo_t = t_svi.t_collapsed_elbo_masked(q, t, y)
...
elbo_t_m = t_svi.t_collapsed_elbo_masked(q, t_mask, y_mask)

print(f"ELBO[masked]  : {elbo_m:.4f}")
print(f"ELBO_t[masked]: {elbo_t_m:.4f}")

mu, Sigma = svi.infer_eps_posterior_single(q, t_mask, y_mask)
mu_t, Sigma_t, _ = t_svi.t_infer_eps_posterior_single(q, t_mask, y_mask)
...
q = collapsed_svi(key, nu=nu, M=12, jitter=1e-6, lengthscale=10.0, variance=1.0, obs_stddev=0.01)


mu, Sigma = svi.infer_eps_posterior_single(q, t, y)
mu_t, Sigma_t, w = t_svi.t_infer_eps_posterior_single(q, t, y)
