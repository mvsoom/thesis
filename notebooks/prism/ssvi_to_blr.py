...
m = opt_posterior.variational_mean.value
...
t = jnp.squeeze(xtest)  # N

Phi = jax.vmap(phi)(t)  # should be (N, M)

mu = Phi @ m  # (N,)

plt.plot(xtest, mu, label="Posterior mean via feature map")
plt.scatter(x, y, alpha=0.15, label="Training Data", color=cols[0])
plt.legend()
