...
def ptform(u):
    z = ndtri(u[:2])
    t = period_norm * u[-2:]  # uniform in [0, 1]
...
def build_gp(theta):
    ...
    return blr_from_mercer(pack, tau, noise_variance=NOISE_FLOOR_POWER)
...
def loglikelihood(x):
    ...
    logl = jax.vmap(
        lambda y, PhiT_y: log_probability(
            ...
            Phi=Phi,
            cov_root=cov_root,
            noise_variance=NOISE_FLOOR_POWER,
            PhiT_Phi=PhiT_Phi,
            PhiT_y=PhiT_y,
            jitter=0.0,
        )
...
for x in xs[:2]:
    ...
    gp = build_gp(theta)
    _, gpc = gp.condition(du_test, tau)

    plt.plot(tau_grid, gp.sample(vk()), label="GP sample")
